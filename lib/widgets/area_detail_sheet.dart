...
class _AreaDetailSheetState extends ConsumerState<AreaDetailSheet> {
  ...
  ({String type, int vertices}) _geomInfo() {
    ...
    if (type == 'Polygon') {
      ...
      final v = (coords as List).fold<int>(
        0,
        (sum, p) => sum + ((p as List)[0] as List).length - 1,
      );
  ...
  Future<void> _delete() async {
    ...
    try {
      await AreaRepository().deleteArea(
        widget.propertyId,
        widget.area.properties.id,
      );
      ref.invalidate(areasProvider(widget.propertyId));
  ...
  Widget build(BuildContext context) {
    ...
    final relevantSubs = _currentCategoryId != null
        ? widget.subcategories
              .where((s) => s.categoryId == _currentCategoryId)
              .toList()
        : <Subcategory>[];
                ...
                children: [
                  ...
                  if (_confirmDelete)
                    Padding(
                      ...
                      child: Text(
                        'Toque novamente para confirmar a exclusão.',
                        style: TextStyle(
                          fontSize: 12,
                  ...
                  OutlinedButton.icon(
                    ...
                    label: Text(
                      _confirmDelete ? 'Confirmar exclusão' : 'Excluir área',
                    ),
