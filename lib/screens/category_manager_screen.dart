  ...
  Widget build(BuildContext context) {
    ...
    return ListView(
      ...
      children: [
        ...
        ...widget.categories.map((cat) => _CategoryCard(
              category: cat,
              ...
              onToggle: () =>
                  setState(() => _expanded[cat.id] = !(_expanded[cat.id] ?? true)),
            )),
      ],
    );
  }
...
class _CategoryCard extends ConsumerWidget {
              ...
              child: Row(children: [
                ...
                if (subcategories.isNotEmpty)
                  Container(
                    ...
                    child: Text('${subcategories.length}',
  ...
  Future<void> _showSubcategoryDialog(
    BuildContext context, WidgetRef ref, Subcategory? editing) async {
    ...
    await showDialog(
      ...
      builder: (_) => AlertDialog(
        title: Text(editing == null ? 'Nova Subcategoria' : 'Editar Subcategoria'),
        content: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          TextField(
            ...
            decoration: const InputDecoration(
                labelText: 'Descrição (opcional)', border: OutlineInputBorder()),
          ),
        ]),
        actions: [
          TextButton(
          ...
          ElevatedButton(
            ...
            onPressed: () async {
              ...
              if (editing == null) {
                ...
                await ref.read(subcategoriesProvider(propertyId).notifier)
                    .create(category.id, name, descCtrl.text.trim().isNotEmpty ? descCtrl.text.trim() : null);
              } else {
                
                await ref.read(subcategoriesProvider(propertyId).notifier)
                    .editSub(editing.id, name, descCtrl.text.trim().isNotEmpty ? descCtrl.text.trim() : null);
              }
            },
        ...
        Padding(
          ...
          child: Row(children: [
            ...
            Container(
              width: 8, height: 8,
              decoration: BoxDecoration(
                color: catColor.withOpacity(0.6), shape: BoxShape.circle),
            ),
            const SizedBox(width: 10),
  ...
  Future<void> _delete(BuildContext context, WidgetRef ref) async {
    final confirm = await showDialog<bool>(
      ...
      builder: (_) => AlertDialog(
        ...
        content: Text('Excluir "${sub.name}"? Áreas associadas perderão a subcategoria.'),
        actions: [
    ...
    if (confirm == true) {
      await ref.read(subcategoriesProvider(propertyId).notifier).deleteSub(sub.id);
    }
  }
}
