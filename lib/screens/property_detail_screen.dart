    ...
    return areasAsync.when(
      ...
      data: (areas) {
        ...
        if (areas.isEmpty) {
          return Center(
            child: Column(mainAxisSize: MainAxisSize.min, children: [
              const Icon(Icons.map_outlined, size: 64, color: Color(0xFFB0BEC5)),
              const SizedBox(height: 12),
              const Text('Nenhuma área cadastrada.',
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          boxShadow: [BoxShadow(blurRadius: 4, color: Colors.black.withOpacity(0.06))],
        ),
          ...
          if (catColor != null && catName != null)
            Row(children: [
              Container(
                width: 9, height: 9,
                decoration: BoxDecoration(
                  color: _hexColor(catColor), shape: BoxShape.circle),
              ),
              const SizedBox(width: 5),
              Expanded(
                child: Text(catName,
    ...
    return ListView(
      ...
      children: [
        ...
        _card(children: [
          ...
          _infoRow('Município / UF', '${property.municipality} — ${property.state}'),
        ...
        _card(children: [
          ...
          _infoRow('Arrendada', '${property.leasedAreaHa.toStringAsFixed(1)} ha'),
          _infoRow('Protegida', '${property.protectedAreaHa.toStringAsFixed(1)} ha'),
...
List<Polygon> _geoToPolygons(Map<String, dynamic> geo, Color fill, Color border, double w) {
  final type = geo['type'] as String;
  final coords = geo['coordinates'] as List;
  if (type == 'Polygon') {
    return [Polygon(points: _ringToLatLngs(coords[0] as List), color: fill, borderColor: border, borderStrokeWidth: w)];
  }
  if (type == 'MultiPolygon') {
    return coords.map((p) => Polygon(
      points: _ringToLatLngs((p as List)[0] as List), color: fill, borderColor: border, borderStrokeWidth: w,
    )).toList();
  }
  return [];
}
...
List<LatLng> _collectPoints(AreaListResponse areas) {
  ...
  for (final f in all) {
    ...
    if (type == 'MultiPolygon') {
      for (final p in coords) pts.addAll(_ringToLatLngs((p as List)[0] as List));
    }
  }
  return pts;
}
